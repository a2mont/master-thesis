...
def extract_data(files: list, drop_zeros: bool = False, dimension='3D'):
    ...
    for file in files:
        ...
        with open(file) as f:
            ...
            for row in reader:
                if drop_zeros:
                    [datas[i][file_id].append(float(row[i])) for i in range(6) if row[i] != 'nan' and not is_zero(row[i])]
                else:
...
def main():
    ...
    q_mins,qualities,topos,contras,inserts,smoothes = extract_data(files, True)
    deltas = {
